...
from im2mesh import onet_vnn
from im2mesh import onet_evn
...
method_dict: Dict[str, ModuleType] = {
    'onet': onet,
    'onet_vnn': onet_vnn,
    'onet_evn': onet_evn
}
...
def get_model(cfg, device=None, dataset=None):
    ...
    '''
    method = cfg['method']
    model: torch.nn.Module = method_dict[method].config.get_model(
...
def get_trainer(model, optimizer, cfg, device):
    ...
    '''
    method = cfg['method']
    trainer: BaseTrainer = method_dict[method].config.get_trainer(
...
def get_dataset(mode, cfg, return_idx=False, return_category=False):
    ...
    if dataset_type == 'Shapes3D':
    ...
    elif dataset_type == 'Shapes3D_registration':
        # Dataset fields
        # Method specific fields (usually correspond to output)
        fields = method_dict[method].config.get_data_fields(mode, cfg)
        # Input fields
        ...
        dataset = data.Shapes3dDatasetRegistration(
            dataset_folder, fields,
            split=split,
            categories=categories,
            rotation_folder=rotation_folder)
        
    elif dataset_type == 'Shapes3D_surface':
        # Dataset fields
        # Method specific fields (usually correspond to output)
        fields = method_dict[method].config.get_data_fields(mode, cfg)
        # Input fields
        ...
        surface_folder = cfg['data']['surface_path']
        dataset = data.Shapes3dDatasetSurface(
            dataset_folder, fields,
            split=split,
            categories=categories,
            rotation_folder=rotation_folder,
...
def get_inputs_field(mode, cfg):
    ...
    if input_type is None:
        ...
        if not 'pointcloud_min_n_rand' in cfg['data'].keys():
            transform = transforms.Compose([
            ...
            transform = transforms.Compose([
                data.RandomSubsamplePointcloud(max_N=cfg['data']['pointcloud_n'],
                                               min_N=cfg['data']['pointcloud_min_n_rand']),
                data.PointcloudNoise(cfg['data']['pointcloud_noise'])
            ])
        with_transforms = cfg['data']['with_transforms']
