...
class RandomSubsamplePointcloud(object):
    ''' Point cloud subsampling transformation class.
    ...
    Args:
        N (int): number of points to be subsampled
    '''
    def __init__(self, max_N: int, min_N: int):
        self.max_N = max_N
        self.min_N = min_N
    ...
    def __call__(self, data):
        ...
        num_points = np.random.randint(low=self.min_N, high=self.max_N+1)
        indices = np.random.randint(points.shape[0], size=num_points)
