...
def solve_R(f1, f2):
    ...
    # logging.info(R)
    diag_1 = torch.tensor(np.concatenate((np.ones([feat_dim_size-1]), np.zeros([1]))), 
                          device=R.device, dtype=R.dtype)
    diag_2 = torch.tensor(np.concatenate((np.zeros([feat_dim_size-1]), np.ones([1]))), 
                          device=R.device, dtype=R.dtype)
